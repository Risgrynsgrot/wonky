...
bool map_load_ldtk_entities(json_object* entities, ldtk_layer_t* layer) {
	...
	for(int i = 0; i < layer->entity_count; ++i) {
		...
		JSON_GET(string, entity_json, __identifier, entity->identifier);
		...
		JSON_GET(string, entity_json, __smartColor, entity->smart_color);
		//NOT FINISHED
	}
	return true;
}
...
bool map_load_ldtk_layers(json_object* layers, ldtk_level_t* level) {
	...
	for(int i = 0; i < level->layer_count; ++i) {
		...
		JSON_GET(string, layer_json, __identifier, layer->identifier);
		JSON_GET(string, layer_json, __type, layer->type);
		...
		JSON_GET(string, layer_json, __tilesetRelPath, layer->tileset_rel_path);
		JSON_GET(string, layer_json, iid, layer->iid);
